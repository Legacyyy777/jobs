...
async def extract_order_number(photo_bytes: bytes) -> Optional[str]:
    ...
    try:
        ...
        # Оптимизированная конфигурация для формата "Заказ-наряд № 123"
        # --psm 6: блок текста (для "Заказ-наряд № 123")
        # --oem 1: LSTM OCR Engine (быстрее)
        config = '--psm 6 --oem 1'
        
        # Распознаём с поддержкой русского (для слова "Заказ")
        text = pytesseract.image_to_string(image, lang='rus+eng', config=config)
        
        logger.info(f"📝 Распознанный текст: {text[:100]}...")
        
        # Ищем номер заказа (приоритет паттерну "Заказ-наряд №")
        patterns = [
            r'[Зз]аказ[-\s]*наряд\s*[№N]\s*(\d{2,6})',  # Заказ-наряд № 123 (приоритет!)
            r'[№N]\s*(\d{2,6})',                         # № 123, №123
            r'наряд\s*[№N]\s*(\d{2,6})',                 # наряд № 123
